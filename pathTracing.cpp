...
RGB calcBsdf(const TipoRayo tipoRayo, const Primitiva* objeto, const Punto& p0) {
    ...
    switch (tipoRayo) {
        case DIFUSO:
            resultado = calcBrdfDifusa(objeto, p0);
            break;
        
        case ESPECULAR:
            resultado = calcBrdfEspecular(objeto->coeficientes.ks);
            break;
        
        case REFRACTANTE:
            resultado = calcBtdf(objeto->coeficientes.kt);
            break;
...
RGB nextEventEstimation(const Punto& p0, const Direccion& normal, const Escena& escena,
    ...
    for (LuzPuntual luz : escena.luces) {
        ...
        RGB reflectanciaBrdfDifusa = calcBrdfDifusa(objOrigen, p0);
...
RGB recursividadRadianciaIndirecta(const Punto& origen, const Direccion &wo, const Direccion& normal,
    ...
    bool hayIntersec = escena.interseccion(wi, ptoIntersec, nuevaNormal, &nuevoObjIntersecado);
...
RGB obtenerRadianciaSalienteIndirecta(const Escena& escena, const unsigned maxRebotes, 
    ...
    for (unsigned i = 0; i < numRayosMontecarlo; ++i) {   // Emisión media de N rayos por Montecarlo
        sumaRadianciasIndirectas += recursividadRadianciaIndirecta(ptoIntersec, wo, normal, escena,
                                                                   maxRebotes, objOrigen);
    }
...
RGB obtenerRadianciaSalienteTotal(const Rayo &rayoIncidente, const Escena &escena, const unsigned maxRebotes, 
    ...
    Primitiva* objIntersec = nullptr;
    
    if (escena.interseccion(rayoIncidente, ptoIntersec, normal, &objIntersec)) {
        RGB powerLuzArea;
        ...
        if (maxRebotes == 0) {  // Solo luz directa
            radianciaSalienteDirecta = nextEventEstimation(ptoIntersec, normal, escena, objIntersec);
...
void renderizarEscena(Camara& camara, unsigned numPxlsAncho, unsigned numPxlsAlto,
    ...
    float tamanoPorPixel = std::min(camara.calcularAnchoPixel(numPxlsAncho), camara.calcularAltoPixel(numPxlsAlto));
    ...
    if(rpp == 1){
        renderizarEscena1RPP(camara, numPxlsAncho, numPxlsAlto, escena, tamanoPorPixel,
                             tamanoPorPixel, maxRebotes, numRayosMontecarlo, coloresEscena,
                             totalPixeles, printPixelesProcesados);
    } else {
        renderizarEscenaConAntialiasing(camara, numPxlsAncho, numPxlsAlto, escena, tamanoPorPixel,
                                        tamanoPorPixel, maxRebotes, numRayosMontecarlo, coloresEscena,
...
void renderizarEscenaConThreads(Camara& camara, unsigned numPxlsAncho, unsigned numPxlsAlto,
    ...
    for (unsigned t = 0; t < numThreads; ++t) {
        ...
        threads.emplace_back(renderizarRangoFilas, std::ref(camara), inicioFila, finFila, numPxlsAncho,
                             std::ref(escena), tamanoPorPixel, tamanoPorPixel, maxRebotes, numRayosMontecarlo,
